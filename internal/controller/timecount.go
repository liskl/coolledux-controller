...
// narrowDigits5x7 is a hand-crafted 5-wide × 7-tall digit bitmap set, as
// row-major bit patterns per digit (row 0 = top). Each row uses bits 4..0
// of a byte; bits 7..5 are zero padding.
var narrowDigits5x7 = [10][7]byte{
	// 0: classic hollow rectangle
	{0b01110,
		0b10001,
		0b10001,
		0b10001,
...
// renderDigitBitmaps96x16 produces the 390-byte digit table expected by the
// firmware's time-count program (content type 0x0a).
//
// TODO: currently unused. The firmware's downsample of the 13x24 bitmap to
// the cell's display resolution is not yet fully understood, so custom
// glyphs packed through this path render as distorted shapes. Re-enable
// once the sampling algorithm is nailed down via /debug/timecount probing.
//
// Layout: 13 cols × 3 bytes per col = 39 bytes per digit, column-major,
// bit 23 = cell row 0 (verified via Probe C).
//
// The 5x7 narrow glyph is packed directly into the bitmap at the left,
// top-padded by 1 row to center vertically in the 9-tall display area.
func renderDigitBitmaps96x16() []byte {
	const (
		digitColumns   = 13
		bytesPerColumn = 3
		bytesPerDigit  = digitColumns * bytesPerColumn // 39
		glyphW         = 5
		glyphH         = 7
		xOffset        = 1 // center 5-wide glyph in 7-wide cell
		yOffset        = 1 // 1 row top padding for 7-tall glyph in 9-tall cell
	)
	out := make([]byte, 0, bytesPerDigit*10)

	for d := 0; d < 10; d++ {
		// Decode the 5x7 row-bits into a bool grid for easy column iteration.
		var pixels [glyphH][glyphW]bool
		for row := 0; row < glyphH; row++ {
			rowBits := narrowDigits5x7[d][row]
		...
		for x := 0; x < digitColumns; x++ {
			colBits := uint32(0)
			srcX := x - xOffset
			if srcX >= 0 && srcX < glyphW {
				for y := 0; y < glyphH; y++ {
					if pixels[y][srcX] {
						colBits |= 1 << (23 - (y + yOffset))
					}
				}
			}
			out = append(out,
